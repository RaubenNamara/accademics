...
// Reset balance to 0 used for clean test
$stmt = $db->prepare("UPDATE leave_balances SET used_days = 0, remaining_days = entitled_days WHERE employee_id = :eid AND leave_type = 'annual' AND year = :year");
...
// Approve the leave request (simulate the PATCH endpoint logic with the fix)
...
// Update balance (simulate the balance update logic with the fix)
